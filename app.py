...
detail_sort_labels = ["Partner", "Bruto", "Cashback", "Líquido", "Leads", "Vendas", "CPL líq.", "CAC líq.", "Lead→Venda"]
detail_sort_keys = ["id_mp", "bruto", "cashback", "liquido", "leads", "vendas", "cpl", "cac", "rate"]
dc1, dc2 = st.columns([2, 1])
with dc1:
    detail_sort_label = st.selectbox("Ordenar por", detail_sort_labels, label_visibility="collapsed", key="detail_sort_col")
with dc2:
    detail_direction = st.segmented_control("Direção", ["Desc", "Asc"], default="Desc",
                                             label_visibility="collapsed", key="detail_sort_dir")
detail_sort_col = detail_sort_keys[detail_sort_labels.index(detail_sort_label)]
detail_df = detail_df.sort_values(detail_sort_col, ascending=(detail_direction == "Asc"), na_position="last")
prev_detail_df = data.build_detail_df(prev_d_ini, prev_d_fim, canal_filter) if compare else None
st.markdown(tables.render_detail_table(detail_df, detail_meds, prev_detail_df, compare), unsafe_allow_html=True)
st.markdown(
    '<div class="legend-row"><span class="sw"><i style="background:var(--crit)"></i></span> '
    'pior que a mediana · <span class="sw"><i style="background:var(--good)"></i></span> melhor que a mediana · '
